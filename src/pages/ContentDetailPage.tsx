...
import { ArrowLeft, Eye, ThumbsUp, Calendar, Clock, Tag, Share2, Printer } from 'lucide-react';

export const ContentDetailPage: React.FC = () => {
  const { blogPosts, precedentDecisions, contentSlug, navigateTo, setContentSlug, language, theme } = useApp();
        ...
        <div style={{ display: 'flex', gap: '40px', flexWrap: 'wrap' }}>
          ...
          <article style={{ flex: '1 1 600px', minWidth: 0 }}>
            ...
            <div style={{
              ...
              <span style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                <Calendar size={14} /> {content.publishedAt}
              </span>
              <span style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
            ...
            <div style={{
              ...
              {content.content.split('\n').map((paragraph, idx) => {
                if (paragraph.startsWith('**') && paragraph.endsWith('**')) {
                  return <h3 key={idx} style={{ color: '#F0DAC5', fontSize: '18px', fontFamily: 'Outfit', margin: '32px 0 12px', fontWeight: 600 }}>{paragraph.replace(/\*\*/g, '')}</h3>;
                }
          ...
          <aside style={{ flex: '0 0 280px', minWidth: '240px' }}>
            <div style={{
              ...
              <h4 style={{ color: '#F0DAC5', fontSize: '14px', fontFamily: 'Outfit', marginBottom: '16px', fontWeight: 600 }}>
                {isEn ? 'Related Content' : 'İlgili İçerikler'}
              </h4>
              {blogPosts
                ...
                .map(related => (
                  ...
                  >
                    ...
                    <span style={{ color: '#718096', fontSize: '11px' }}>{related.publishedAt}</span>
                  </div>
                ))
              }
